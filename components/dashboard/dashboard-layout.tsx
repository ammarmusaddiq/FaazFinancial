...
import { Home, Building2, FileText, CreditCard, Settings, HelpCircle, LogOut, Menu, X, User, Bell } from "lucide-react"
...
export function DashboardLayout({ children, user, profile }: DashboardLayoutProps) {
  ...
  const navigation = [
    ...
    { name: "My Businesses", href: "/dashboard/businesses", icon: Building2, current: false },
            ...
            {navigation.map((item) => (
              <Link
                ...
                className={`flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                  item.current ? "bg-cyan-100 text-cyan-700" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"
                }`}
              >
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center space-x-2">
                    <Avatar className="h-8 w-8">
                      ...
                      <AvatarFallback className="bg-cyan-100 text-cyan-700">{userInitials}</AvatarFallback>
                    </Avatar>
